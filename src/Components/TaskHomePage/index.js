...
class TaskHomePage extends Component {
    ...
    const defaultTasks = [
      {
        ...
        title: "project 1",
        description:
          "this project is about to display the list of tasks of user",
        teamSize: 1,
        mode: "Start",
      },
      {
  ...
  handleDeleteTask = (taskId) => {
    const updatedTaskDetails = this.state.taskDetails.filter(
      (task) => task.id !== taskId
    );
    this.setState(
      {
        taskDetails: updatedTaskDetails,
        successMessage: "Your task is deleted successfully",
      },
      () => {
  ...
  handleModeButtonClick = (mode) => {
    this.setState({ taskMode: mode }, () => {
      if (mode === "") {
        ...
        const filteredTasks = this.state.taskDetails.filter(
          (task) => task.mode === mode
        );
        this.setState({ filteredTasks });
      }
        ...
        <div className="home-content-part">
          ...
          <div className="tabs-container">
            <button
              type="button"
              className="tab-btn"
              onClick={() => this.handleModeButtonClick("")}
            >
              All
            </button>
            <button
              type="button"
              className="tab-btn"
              onClick={() => this.handleModeButtonClick("Start")}
            >
              Start
            </button>
            <button
              type="button"
              className="tab-btn"
              onClick={() => this.handleModeButtonClick("End")}
            >
              End
            </button>
            <button
              type="button"
              className="tab-btn"
              onClick={() => this.handleModeButtonClick("Complete")}
            >
              Complete
            </button>
          </div>
          <div className="tasks-container">
            <ul className="task-items-list">
              {filteredTasks.map((eachTask) => (
                <TaskItem
                  key={eachTask.id}
                  taskItemDetails={eachTask}
                  onTaskClick={this.handleTaskClick}
                  onTaskDelete={this.handleDeleteTask}
                  onTaskUpdate={this.handleUpdateTaskClick}
                />
              ))}
            </ul>
          </div>
          {isAddingTask || isUpdatingTask ? (
            <div className="task-form">
              ...
              <div className="form-group">
                <label htmlFor="teamSize">Team Name</label>
                <br />
                <input
                  type="text"
                  id="teamSize"
                ...
                />
                {errors.teamSize && (
                  <span className="error">{errors.teamSize}</span>
                )}
              </div>
              <div className="form-group">
                <label htmlFor="taskModeDropDown">Task Mode</label>
                <br />
                <select
                  id="taskModeDropDown"
                  value={taskMode}
                  className="dropdown"
                  onChange={(e) => this.setState({ taskMode: e.target.value })}
                >
  ...
  render() {
    ...
    return isSelectTask ? (
      <SummaryPage task={selectedTask} onClickBack={this.onClickBack} />
    ) : (
      this.renderTaskHome()
    );
  }
